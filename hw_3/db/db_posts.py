...
def update_post(post_id, title, description, tags, is_private):
    ...
    cursor = connection.cursor()
    try:
        cursor.execute(f"UPDATE posts SET title=\'{title}\', description=\'{description}\', tags=\'{tags}\', is_private=\'{is_private}\', update_date=\'{str(datetime.datetime.now())}\' id = \'{post_id}\';")
        connection.commit()
        return True
    except: return False
