...
def check_time(token):
    ...
    return now > modified + timedelta(minutes=10)
...
def get_user_id_cred(login, password):
    cursor = connection.cursor()
    cursor.execute(f"SELECT user_id from service_users WHERE user_login = \'{login}\' and user_pass = \'{get_password_encrypt(password)}\';")
    connection.commit()
    result = cursor.fetchone()
    return result[0]

print(get_user_id_cred("svkle", "test"))